...
The **Subjects** workspace already acts as the class control center, so its next design pass should prioritize separating the selected Subject identity from its operation modules while preserving the current direct management flow. **Attendance** should retain the stronger review-before-publish emphasis introduced by the PRD work, with statuses and unresolved suggestions using a single consistent operational alert treatment. The shared preparation-and-action footer is intentionally used for Subject setup and private Zoom intake, where a compact decision boundary improves clarity; **Content** keeps its tailored draft, preview, media, and publication flow instead of duplicating that footer inside a richer authoring surface.
