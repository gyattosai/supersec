...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-background text-foreground"><aside className="fixed inset-y-0 left-0 hidden w-72 flex-col border-r border-sidebar-border bg-sidebar lg:flex"><Link href="/app" className="flex items-center gap-3 border-b border-sidebar-border px-6 py-6 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><span className="grid h-10 w-10 place-items-center rounded-[14px] bg-primary text-xs font-extrabold text-primary-foreground">SS</span><span><span className="block font-[Manrope] text-base font-extrabold tracking-[-0.05em]">supersec</span><span className="mt-0.5 block text-xs text-muted-foreground">A class secretary management system</span></span></Link><div className="px-4 pb-3 pt-7"><p className="signal-kicker px-3">Work areas</p><div className="mt-3"><NavItems /></div></div><div className="mt-auto border-t border-sidebar-border px-6 py-5"><p className="signal-kicker">Signed in</p><p className="mt-2 truncate text-sm font-semibold">{user.name || "Class secretary"}</p><p className="mt-1 truncate text-xs text-muted-foreground">{user.email || "Workspace owner"}</p><Button onClick={logout} variant="ghost" className="mt-4 w-full justify-start px-2 text-muted-foreground hover:text-foreground"><LogOut className="h-4 w-4" />Sign out</Button></div></aside>
