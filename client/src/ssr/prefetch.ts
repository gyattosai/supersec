...
export type SsrPrefetch = { publicSubject: (publicId: string) => Promise<any>; publicItem: (input: { kind: "announcement" | "resource" | "question"; publicId: string }) => Promise<any>; publicReport: (publicId: string) => Promise<any> };
...
export async function prefetchForPath(url: string, queryClient: QueryClient, caller: SsrPrefetch): Promise<HeadMeta> {
  ...
  if (path === "/app" || path.startsWith("/app/") || path.startsWith("/attendance/")) return { title: site, description: fallback, noindex: true };
