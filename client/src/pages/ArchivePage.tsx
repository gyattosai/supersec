...
export default function ArchivePage() {
  ...
  return <DashboardLayout><section className="mx-auto max-w-5xl"><WorkspacePageHeader eyebrow="Archive" title="Archive" description="Restore any item as a draft. Nothing is deleted here." /><div className="mt-6 grid gap-3 sm:grid-cols-3"><ArchiveMetric label="Subjects" value={archivedSubjects.length} /><ArchiveMetric label="Reports" value={archivedReports.length} /><ArchiveMetric label="Content" value={archivedContentCount} /></div><section className="mt-7"><ArchiveSection icon={FileArchive} label="Subjects">{archivedSubjects.map(subject => <ArchiveCard key={subject.id} icon={BookOpen} title={subject.name} detail={`${subject.code} · ${subject.professorName}`} onRestore={() => restoreSubject.mutate({ subjectId: subject.id, archive: false })} busy={busy} />)}{!archivedSubjects.length ? <EmptyArchive label="No archived Subjects." /> : null}</ArchiveSection></section><section className="mt-9"><ArchiveSection icon={ArchiveRestore} label="Reports">{archivedReports.map(report => <ArchiveCard key={report.id} icon={FileArchive} title={report.reportType === "class_attendance" ? "Class Attendance" : "All Subject Attendance"} detail={report.reportType === "class_attendance" ? `${report.subjectCode ?? "Subject"}${report.subjectName ? ` · ${report.subjectName}` : ""}${report.sessionStartsAt ? ` · ${formatDateTime12Hour(report.sessionStartsAt)}` : ""}` : `All active Subjects · Generated ${new Date(report.generatedAt).toLocaleDateString()}`} version={report.version} onRestore={() => restoreReport.mutate({ id: report.id })} busy={busy} />)}{!archivedReports.length ? <EmptyArchive label="No archived reports." /> : null}</ArchiveSection></section><section className="mt-9"><ArchiveSection icon={Megaphone} label="Content">{content.data?.announcements.map(item => <ArchiveCard key={`a-${item.id}`} icon={Megaphone} title={item.title} detail={`Announcement · ${item.subjectName}`} version={item.version} onRestore={() => restoreAnnouncement.mutate({ id: item.id })} busy={busy} />)}{content.data?.resources.map(item => <ArchiveCard key={`r-${item.id}`} icon={BookOpen} title={item.title} detail={`Resource · ${item.subjectName}`} version={item.version} onRestore={() => restoreResource.mutate({ id: item.id })} busy={busy} />)}{content.data?.questions.map(item => <ArchiveCard key={`q-${item.id}`} icon={MessageCircleQuestion} title={item.title} detail={`Question & Answer · ${item.subjectName}`} version={item.version} onRestore={() => restoreQuestion.mutate({ id: item.id })} busy={busy} />)}{!archivedContentCount ? <EmptyArchive label="No archived announcements, resources, or Questions & Answers." /> : null}</ArchiveSection></section></section></DashboardLayout>;
}
