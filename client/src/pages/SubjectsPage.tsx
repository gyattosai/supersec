...
import { Archive, BookOpen, CalendarDays, Check, CircleAlert, CircleHelp, Clipboard, GraduationCap, Megaphone, Plus, UserMinus, UserPlus, Users } from "lucide-react";
...
function SubjectWorkspace({ subject }: { subject: { id: number; publicId: string; name: string; code: string; professorName: string; publishState: "draft" | "published"; status: "active" | "archived"; meetingDays: Array<{ weekday: number; startTime: string | null; endTime: string | null }> } }) {
  ...
  const [studentName, setStudentName] = useState(""); const [sessionAt, setSessionAt] = useState(""); const [noClassDialog, setNoClassDialog] = useState<{ id: number; startsAt: Date; reason: string } | null>(null); const [createNoClassOpen, setCreateNoClassOpen] = useState(false);
