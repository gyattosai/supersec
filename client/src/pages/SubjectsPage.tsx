...
export default function SubjectsPage() {
  ...
  return <DashboardLayout><section className="mx-auto max-w-7xl">
    <WorkspacePageHeader eyebrow="Private workspace" title="Subjects" description="Open a separate class space for every roster, Schedule, Attendance record, and shared update." action={<Link href="/app/subjects/new" className="inline-flex min-h-11 items-center gap-2 rounded-2xl bg-primary px-4 text-sm font-semibold text-primary-foreground shadow-[0_12px_24px_-16px_rgba(199,82,0,0.8)] transition-transform duration-300 [transition-timing-function:cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-0.5 active:scale-[0.98]"><Plus data-icon="inline-start" />New Subject</Link>} />
    <section className="mt-8">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">Class gallery</p><h2 className="mt-2 text-2xl font-semibold tracking-[-0.03em]">Choose the class you need to run.</h2></div><Badge variant="secondary" className="rounded-full px-3 py-1">{activeCount} active</Badge></div>
      ...
      <div className="mt-6 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{subjects.data?.filter(subject => subject.status === "active").map(subject => <SubjectGalleryCard key={subject.id} subject={subject} />)}</div>
    </section>
...
function SubjectGalleryCard({ subject }: { subject: { id: number; name: string; code: string; professorName: string; termName: string | null; publishState: "draft" | "published"; meetingDays: Array<{ weekday: number; startTime: string | null; endTime: string | null }> } }) {
  return <Link href={`/app/subjects/${subject.id}`} className="group rounded-[30px] bg-secondary/35 p-1 ring-1 ring-border/75 transition-[transform,box-shadow] duration-300 [transition-timing-function:cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-1 hover:shadow-[0_20px_40px_-30px_rgba(199,82,0,0.9)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><article className="min-h-64 rounded-[26px] bg-card p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] sm:p-6"><div className="flex items-start justify-between gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-primary text-primary-foreground"><BookOpen /></span><RecordStatusBadge tone={subject.publishState === "published" ? "published" : "draft"}>{subject.publishState}</RecordStatusBadge></div><p className="mt-6 text-xs font-semibold uppercase tracking-[0.14em] text-primary">{subject.code}</p><h3 className="mt-2 text-2xl font-semibold tracking-[-0.035em]">{subject.name}</h3><p className="mt-2 text-sm text-muted-foreground">{subject.professorName}{subject.termName ? ` · ${subject.termName}` : ""}</p><div className="mt-6 flex items-start gap-2 text-sm leading-6 text-muted-foreground"><CalendarDays className="mt-0.5 shrink-0 text-primary" /><span>{formatDays(subject.meetingDays)}</span></div><span className="mt-6 inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-primary">Open class space<ArrowRight data-icon="inline-end" className="transition-transform group-hover:translate-x-0.5" /></span></article></Link>;
}
...
function EmptyGallery() { return <div className="mt-6 grid min-h-80 place-items-center rounded-[30px] border border-dashed border-border bg-card p-8 text-center"><div><span className="mx-auto grid size-12 place-items-center rounded-2xl bg-secondary text-primary"><GraduationCap /></span><h2 className="mt-5 text-xl font-semibold">Create your first Subject</h2><p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-muted-foreground">Each Subject gets its own Students, Schedule, class sessions, Attendance, and sharing space.</p><Link href="/app/subjects/new" className="mt-6 inline-flex min-h-11 items-center gap-2 rounded-2xl bg-primary px-4 text-sm font-semibold text-primary-foreground"><Plus data-icon="inline-start" />New Subject</Link></div></div>; }
