...
- **Content:**
  ...
  - **Format:** `.html`, `.css`, `.tsx`
  - **Notes:** The selection persists at the `theme` local-storage key after hydration. Public and private headers use the same toggle; dark remains the default. The dark canvas is charcoal `#151619` with a deliberate grey surface ladder; ambient canvas gradients and opaque elevated-surface gradients preserve content contrast. Apply after the registry’s initial artifact card creates `.project/artifacts.md`.
  ```
